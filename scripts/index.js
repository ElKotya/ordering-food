...
const renderCounter = () => {
  ...
  const counter = basketList.reduce((acc, item) => {
  ...
  products.forEach((item) => {
    ...
    newDiv.addEventListener('click', ({target}) => {
      ...
      if (className === 'products-card__bottom-btn') {
        const findedItem = basketList.find((basketItem) => item.id === basketItem.id);
        if(!findedItem) {
...
const renderBasketList = () => {
  ...
  const list = basketList.map((item) => {
    const newDiv = document.createElement('div');
    newDiv.className = 'products-card';
    newDiv.innerHTML = `
      <img src="${item.preview}" alt="" class="products-card__preview">
    ...
    newDiv.addEventListener('click', ({target}) => {
      ...
      if (className === 'card__count-plus') {
        ...
        removeElement(target.parentNode.parentNode);
        const filteredList = basketList.filter((el) => item.id !== el.id);
        basketList = filteredList;
        renderBasketList();
        renderCounter();
      }
      localStorage.setItem('basket', JSON.stringify(basketList));
